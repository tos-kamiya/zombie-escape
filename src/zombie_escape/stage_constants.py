...
def _build_stage36_reinforced_wall_zones() -> list[tuple[int, int, int, int]]:
    ...
    # Extend walls up to the row right below the top storage area.
    min_y = 4
    # Stop right above the bottom storage area.
    max_y = 18
    doorway_rows = {4, 18}
...
def _build_stage36_storage_material_spawns(
    *, grid_cols: int, grid_rows: int, band_height: int
    ...
    """Fill top/bottom storage bands with materials, skipping outer border cells."""
    if band_height <= 0:
        return []
    ...
    # Keep left/right edge corridors open: skip 3 cells on each side.
    start_x = 3
