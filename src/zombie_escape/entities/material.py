...
from ..entities_constants import MATERIAL_COLLISION_RADIUS, MATERIAL_SIZE
...
class Material(pygame.sprite.Sprite):
    ...
    def __init__(self: Self, x: float, y: float) -> None:
        super().__init__()
        self.image = pygame.Surface((MATERIAL_SIZE, MATERIAL_SIZE), pygame.SRCALPHA)
        self.image.fill((182, 152, 108))
        pygame.draw.rect(self.image, (70, 56, 30), self.image.get_rect(), width=1)
        self.rect = self.image.get_rect(center=(int(x), int(y)))
        self.x = float(self.rect.centerx)
        self.y = float(self.rect.centery)
        self.collision_radius = float(MATERIAL_COLLISION_RADIUS)
