...
-   **時間加速 (Stage 5):** `Shift` キーを押し続けると、世界と夜明けタイマーが5倍速で進み、離すと元の速度に戻ります。
...
- **Stage 5: Survive Until Dawn** — どの車も燃料切れ。HUD には約4時間ぶんに換算した見掛けのタイマーが表示されるが、実際は30分間耐え切れば夜が明ける。`Shift` キーを押せばゲーム全体とタイマーを5倍速で進められる。夜明けになると屋外のゾンビは炭化し、既存の出口から徒歩で屋外に出ればクリア（車での脱出はできない）。崩壊する安全地帯を演出するため、画面外でプレイヤーの近くにも新たなゾンビが湧き続ける。
