...
-   **鉄骨（難しい）:** 5%の確率で単マスの障害物を配置します。内壁と重なっている場合は内壁破壊後に見えるようになります。
...
- **Stage 3: Rescue Buddy** — 燃料を拾い、相棒を車で回収し、一緒に脱出する。
...
-   **相棒 (Stage 3):** 緑の丸で表示される仲間。ステージ内のどこかに座り込んでいます。
    -   画面内にいるときだけゾンビが狙います。画面外で捕まった場合はゲームオーバーにならず、別の場所に静かに再出現します。
