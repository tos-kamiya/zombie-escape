...
def test_ui_json_schemas_are_synchronized() -> None:
    ...
    locales_dir = Path(__file__).resolve().parents[1] / "src" / "zombie_escape" / "locales"
    json_files = list(locales_dir.glob("ui.*.json"))
    assert len(json_files) >= 2, f"Expected at least 2 ui.*.json files, found {len(json_files)}"
    ...
    for path in json_files:
        ...
        assert len(data) == 1, f"Expected exactly one top-level key in {path.name}, found {list(data.keys())}"
    ...
    for lang, data in file_data.items():
        ...
        if missing_in_current:
            errors.append(f"Missing keys in '{lang}' (present in '{reference_lang}'): {sorted(missing_in_current)}")
        if extra_in_current:
            errors.append(f"Extra keys in '{lang}' (missing in '{reference_lang}'): {sorted(extra_in_current)}")
            
        assert not errors, f"Schema mismatch between '{reference_lang}' and '{lang}':\n" + "\n".join(errors)
...
def test_ui_json_mandatory_sections() -> None:
    ...
    mandatory_sections = {"meta", "game", "fonts", "menu", "stages", "status", "hud", "game_over", "settings", "errors"}
