...
- **捕獲メカニズム**:
  ...
  - 観葉植物が破壊されても、既に生成済みの捕獲体は解放されない。
- **捕獲体 (`TrappedZombie`) の性質**:
  - 見た目は捕獲状態用アウトライン（紫系）で表示される。
  - 移動しないが脅威判定には残るため、接触したプレイヤー/生存者の危険は継続する。
...
- **AI/判定補足**:
  - 巡回ロボットは観葉植物を障害物として扱う。
  - 接触判定はセル中心前提を利用し、近傍セル（3x3）探索で計算量を抑えている。
...
- **移動への影響**:
  - 水たまりセル上にいる間、移動速度が `PUDDLE_SPEED_FACTOR`（既定 0.3）倍になる。
  - 対象はプレイヤー（徒歩/車）、生存者/相棒、ゾンビ、ゾンビ犬、巡回ロボット。
  - 観葉植物接触による減速が優先され、水たまり減速は「観葉植物非接触時のみ」適用される。
- **描画**:
  - 通常画面では床タイルとして描画され、位相付きの波紋リングでアニメーションする。
  - ゲームオーバー縮小図/デバッグ全体図でも水たまりリングが可視化される。
