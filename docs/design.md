...
- F キーでフルスクリーン/ウィンドウをトグル（起動時は常にウィンドウ）。
- フルスクリーンは SDL2 Window 経由で、現在のウィンドウ位置からディスプレイ番号を推定して切替を試みる。
  - 取得できない場合は SDL2 の `get_display_index()` を利用し、それも失敗すれば通常の `set_mode(FULLSCREEN)` にフォールバックする。
- 解除時は直前のウィンドウ位置へ戻す（Wayland 等では位置指定が無視される場合がある）。
- 論理解像度は 400x300 のまま、OSのウィンドウサイズに合わせて拡大描画する。
