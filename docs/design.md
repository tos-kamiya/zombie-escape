...
- ゾンビ/ゾンビ犬はロボット接触中に麻痺し、ロボットから離れて一定時間（ms）経過するまで停止する。
...
- 対象はプレイヤー/生存者/相棒/ゾンビ/ゾンビ犬。車・巡回ロボットは影響を受けない。
- 発動条件は「キャラクタの当たり判定が床タイル内に完全に収まったとき」。
- 発動中はキャラクタの移動入力/AIは無効化され、床の移動方向にのみ動く。
- 床の移動速度はプレイヤー速度の約2倍で、即時ワープではない。
- 床タイルの端に到達したら、はみ出さないギリギリで停止する。
- 床効果の終了条件は「床から少しでもはみ出したとき」（入る条件と対称）。
- ステージ定義で床の位置と向きを指定し、落とし穴と重なる場合は床が優先される。
- 描画は灰色タイル＋`U/D/L/R` の文字。
