...
- `screens/title.py`
  - タイトルメニュー、ステージセレクト、シード入力。
  - **ステージセレクトの改良**: クリア済みステージの横に、そのステージの特性（登場キャラクタやギミック）を示すミニアイコンを表示。
    - アイコンは `render_assets.get_character_icon` / `get_tile_icon` で生成。
    - キャラクタ識別: 重要人物（プレイヤー/相棒）は腕を描き、それ以外（生存者/ゾンビ）は腕を描かないルールで統一。
    - ゾンビ変種: アイコン右下に一文字（T: 追跡, W: 壁沿い, L: 列形成）を付与して識別。
    - 特殊状態: 耐久ステージ（車不可）では×印付きの車アイコンを表示。
    - 地形ギミック: 落とし穴、落下スポーン床、動く床のアイコンも表示。
    - レイアウト: 選択行に応じて説明文の位置を上下に動的にシフトし、長いアイコン列との重なりを回避。
    - ハイライト: 選択状態の背景を左カラムから右ペイン端まで拡張（ステージ選択時）。
...
### 4.7 トゲトゲした観葉植物 (Spiky Houseplant)

- **概要**: 
  - セルの中心付近に配置される動かないトラップ型エンティティ。
  - ゾンビやゾンビ犬を「捕獲」してその場に固定する機能を持つ。
- **耐久力**: 
  - 通常のゾンビと同程度の体力を持ち、車で撥ねることで破壊可能。
- **捕獲メカニズム**:
  - 接触したゾンビ・ゾンビ犬をその場に固定し、移動速度を0にする。
  - ひとつの植物に複数のゾンビが同時に捕まることが可能。
  - 植物が破壊されても、既に捕まっているゾンビは解放されず、その場に固定されたまま残る。
- **捕まったゾンビの状態**:
  - **外観**: 識別のため、境界線（アウトライン）が通常の赤から**紫色**に変化する。
  - **Health/Decay**: 捕獲時のHealthを引き継ぎ、時間経過（Decay）によって最終的に消滅する。
  - **危険性**: 静止しているが、プレイヤー、生存者、相棒が触れると通常通りゾンビ化（ミス）する。
  - **衝突**: 他の動いているゾンビにとっては障害物（壁）として機能し、反発を発生させる。
  - **車との相互作用**: 自動車が通過すると植物は破壊される。
- **人間・車両への影響**:
  - プレイヤー（徒歩）、生存者、相棒、および自動車は、この植物の上を通過できるが、接触している間は移動速度が大幅に（0.2倍に）低下する。
- **AIの挙動**:
  - パトロールロボットや相棒は、この植物があるセルを障害物として認識し、自動的に避ける。
  - ゾンビ犬は、ターゲットのゾンビが捕獲されている場合、それを追従対象から除外し、既に追従中の場合は解除する。
- **最適化**:
  - セルの中心付近に配置される制約を利用し、隣接セル判定のみでインタラクションを処理することで計算負荷を抑える。

### 4.8 水たまり (Puddle)

- **概要**:
  - 地形タイル（エンティティではない）として配置される減速ギミック。
  - ブループリントでは `w` 文字で表現される。
- **移動への影響**:
  - 水たまりセル上にいる間、移動速度が `PUDDLE_SPEED_FACTOR`（既定 0.2）倍になる。
  - 対象はプレイヤー（徒歩/車）、生存者、ゾンビ、ゾンビ犬、巡回ロボット。
  - 観葉植物接触による減速が優先され、水たまり減速は「観葉植物非接触時のみ」適用される。
- **描画**:
  - 通常画面では床タイルとして描画され、簡易的な波紋アニメーションを持つ。
  - ゲームオーバー縮小図/デバッグ全体図でも専用色で可視化される。
...
- 壁沿い移動 (`zombie_wall_hug_movement`)
  - 視界範囲 `ZOMBIE_TRACKER_SIGHT_RANGE` でプレイヤーを検知したら直進追尾。
  - 視界外は前方と±45度のプローブで壁までの距離を測り、目標ギャップを維持するよう旋回する。
  - **安定性向上のための調整**:
    - **セルサイズ依存のセンサー距離**: センサー（プローブ）の長さはセルサイズに応じて動的に決定される（`max(24, cell_size * 0.6)`）。これにより 35px から 60px までの異なるグリッドサイズでも安定して角を曲がることが可能。
    - **幾何学的補正**: 斜め45度のプローブで計測される距離の誤差を補正するため、目標ギャップ（`ZOMBIE_WALL_HUG_TARGET_GAP`）を 5.5px に最適化。
    - **旋回強度の最適化**: 1フレームあたりの旋回角度を 6.0度（`ZOMBIE_WALL_HUG_TURN_STEP_DEG`）に調整し、直線での安定性とコーナーでの追従性を両立。
...
- `generate_random_blueprint(...)`
  - 外周 -> 出口 -> スポーン・アイテム候補（P/C/e/f/l/s、必要数はステージ設定から指定）予約 -> 落とし穴/水たまり/観葉植物 -> 壁 -> 鉄筋候補 の順に生成。
  - 予約地点（`reserved_cells`）には壁・落とし穴・水たまり・観葉植物が生成されないよう保護される。
  - `fall_spawn_zones` は地形文字を直接上書きせず、後段で `fall_spawn_cells` として別管理される（他ギミック床との重なりを許可）。
